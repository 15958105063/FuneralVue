...
namespace Funeral.Core.Services
{
    public partial class AchUsrServices : BaseServices<AchUsr>, IAchUsrServices
    {
        ...
        private readonly IAchRacRepository _achRacRepository;
        public AchUsrServices(IAchRacRepository achRacRepository, IAchUsrRepository dal, IHostingEnvironment hostingEnvironment)
        {
            _environment = hostingEnvironment;
            this._dal = dal;
            base.BaseDal = dal;
            this._achRacRepository = achRacRepository;
        }
        ...
        public async Task<string> SaveWordFile(string savePath, string tablename,string linktablename, int tid)
        {
            tablename = tablename.Replace("Ach", "Ful");
            linktablename = linktablename.Replace("Ach", "Ful");
            savePath = Appsettings.app(new string[] { "WordDownLoadUrl", "Url" });//获取连接字符串
            ...
            try
            {
                ...
                using (var stream = new FileStream(Path.Combine(uploadPath, fileName), FileMode.Create, FileAccess.Write))
                {
                    ...
                    #region 删除语句
                    //先操作删除语句
                    document.SetParagraph(NpoiWordParagraphTextStyleHelper._.ParagraphInstanceSetting(document, $"DELETE FROM {tablename};", false, 7, "宋体", ParagraphAlignment.LEFT), 0);
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        document.SetParagraph(NpoiWordParagraphTextStyleHelper._.ParagraphInstanceSetting(document, $"INSERT INTO {tablename} ({insertstring}) VALUES", false, 7 ,"宋体", ParagraphAlignment.LEFT), 1);
                    }
                    #endregion
                    ...
                    foreach (var item in list)
                    {
                        ...
                        if (list.Count == index - 1)
                        {
                            document.SetParagraph(NpoiWordParagraphTextStyleHelper._.ParagraphInstanceSetting(document, $"({valuesstring});", false, 7, "宋体", ParagraphAlignment.LEFT), index);
                    ...
                    int index2 = index+1;
                    var list2 = await _achRacRepository.Query(x => x.Tid == tid);

                    #region 删除语句
                    //先操作删除语句
                    document.SetParagraph(NpoiWordParagraphTextStyleHelper._.ParagraphInstanceSetting(document, $"DELETE FROM {linktablename};", false, 8, "宋体", ParagraphAlignment.LEFT), index2++);
                    ...
                    if (list2.Count > 0)
                    {
                        ...
                        document.SetParagraph(NpoiWordParagraphTextStyleHelper._.ParagraphInstanceSetting(document, $"INSERT INTO {linktablename} ({insertstring}) VALUES", false, 8, "宋体", ParagraphAlignment.LEFT), index2++);
                    }
                    #endregion
                    ...
                    foreach (var item in list2)
                    {
                        ...
                        valuesstring = valuesstring.Substring(0, valuesstring.Length - 1);
                        index3++;
                        if (list2.Count == index3)
                        {
                            document.SetParagraph(NpoiWordParagraphTextStyleHelper._.ParagraphInstanceSetting(document, $"({valuesstring});", false, 8, "宋体", ParagraphAlignment.LEFT), index2);
