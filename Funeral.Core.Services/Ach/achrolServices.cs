...
namespace Funeral.Core.Services
{
    ...
    {
        private readonly IAchAcsRepository _achAcsRepository;
        private static IHostingEnvironment _environment;
        private readonly IAchRolRepository _dal;
        public AchRolServices(IAchAcsRepository achAcsRepository, IAchRolRepository dal, IHostingEnvironment hostingEnvironment)
        {
            _achAcsRepository = achAcsRepository;
        ...
        public async Task<string> SaveWordFile(string savePath, string tablename, string linktablename, int tid)
        {
            ...
            try
            {
                ...
                using (var stream = new FileStream(Path.Combine(uploadPath, fileName), FileMode.Create, FileAccess.Write))
                {
                    ...
                    int index2 = index + 1;
                    var list2 = await _achAcsRepository.Query(x => x.Tid == tid);
